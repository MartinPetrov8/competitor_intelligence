...
def test_health_endpoint_with_scraper_data(client: FlaskClient, tmp_path: Path) -> None:
    """Test /health endpoint returns correct scraper count and last_run timestamp."""
    db_path = tmp_path / "dashboard.db"
    
    # Set up test database with sample competitors and scraped_at data
    cid1 = _competitor_id(db_path, "onwardticket.com")
    cid2 = _competitor_id(db_path, "vizafly.com")
    
    with sqlite3.connect(db_path) as conn:
        # Insert data with different timestamps to test MAX(scraped_at)
        conn.execute(
            """
            INSERT INTO prices_v2 (
                competitor_id, scrape_date, scraped_at, main_price, currency,
            ...
            """,
            (cid1, "2026-02-22", "2026-02-22T10:00:00Z", 12.5, "USD", "[]", "https://onwardticket.com"),
        )
        conn.execute(
            """
            INSERT INTO products_v2 (
            ...
            """,
            (cid2, "2026-02-22", "2026-02-22T15:30:00Z", 1, 15.0, 0, None, 0, None, 0, None, "https://vizafly.com"),
        )
        conn.execute(
            """
            INSERT INTO reviews_trustpilot (
                competitor_id, scrape_date, scraped_at, overall_rating, total_reviews,
                rating_1, rating_2, rating_3, rating_4, rating_5, source_url
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (cid1, "2026-02-22", "2026-02-22T16:45:00Z", 4.5, 200, 10, 5, 15, 50, 120, "https://trustpilot.com/review/onwardticket.com"),
        )
        conn.commit()
    ...
    # Assert response JSON has correct structure (status, scrapers, last_run keys)
    ...
    # Verify scrapers count is correct (5 competitors seeded by init_database)
    assert data["scrapers"] == 5
    
    # Verify last_run timestamp is the most recent scraped_at (reviews_trustpilot has latest)
    assert data["last_run"] == "2026-02-22T16:45:00Z"
